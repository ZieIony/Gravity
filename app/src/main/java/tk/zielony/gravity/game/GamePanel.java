...
public class GamePanel extends View implements SensorEventListener {
	...
	List<BlackHole> holes = new ArrayList<BlackHole>();
	...
	public void onSensorChanged(SensorEvent event) {
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			if (event.values[2] < -9)
				fireAchievementEvent(AchievementType.UPSIDE_DOWN);
		}
	}
	...
	protected void dispatchDraw(Canvas canvas) {
		...
		if (prevTime != 0) {
			...
			if (frames == 10) {
				...
				if (fps < 10)
					fireAchievementEvent(AchievementType.SLOW_JAVA);
				frameTime = 0;
			}
			frames %= 10;
		...
		if (timeElapsed > 20 * 60)
			fireAchievementEvent(AchievementType.GOOD_GAME);

		if (timeElapsed > 60 && apm < 5)
			fireAchievementEvent(AchievementType.LOW_APM);

		if (timeElapsed > 60 && apm > 1000)
			fireAchievementEvent(AchievementType.HIGH_APM);
	...
	private void doPhysics() {
		...
		for (Planet p : planets) {
			if (p.mass > 8000 && Math.random() < 0.001) {
		...
		for (int i=0;i<spaceShips.size();i++) {
			...
			if (spaceShips.size()>1&&Math.random() < 0.02f) {
				...
				lasers.add(new Laser(s.x, s.y, p.x, p.y, s.color));
			}
			if (s.x < MIN_X * getWidth() || s.x > MAX_X * getWidth()
					|| s.y < MIN_Y * getHeight() || s.y > MAX_Y * getHeight())
				SpaceShip.toRemove.add(s);
		}
		...
		for (Laser l : lasers) {
			...
			float shipRadius = 5;
			for (SpaceShip s : spaceShips) {
				if (dist(s.x, s.y, l.x, l.y) < shipRadius) {
					Laser.toRemove.add(l);
					SpaceShip.toRemove.add(s);
					explosions.add(new Explosion(s.x, s.y, shipRadius));
					break;
				}
			}
		...
		for (Planet p : planets) {
			if (Math.random() < 0.0001f)
				spaceShips.add(new SpaceShip(p.x, p.y, p.velx, p.vely));
		...
		spaceShips.removeAll(SpaceShip.toRemove);
			...
			if (event.getPointerCount() == 1) {
				...
				if (tapStarted && event.getAction() == MotionEvent.ACTION_UP) {
					...
					if (r < 0.01) {
						...
						p.setRadius(15);
						fireAchievementEvent(AchievementType.CREATE_SUN);
					} else if (r < 0.02) {
						p.color = 0xff559B27;
						p.color2 = 0xff00A5FF;
						p.label = "The Earth " + random.nextInt(50);
						p.setRadius(7);
						fireAchievementEvent(AchievementType.CREATE_EARTH);
					} else if (r < 0.03) {
						p.color = 0xffFF3330;
						p.color2 = 0xffFF3330;
						p.label = "Mars " + random.nextInt(50);
						p.setRadius(7);
						fireAchievementEvent(AchievementType.CREATE_MARS);
					} else if (r < 0.04) {
						p.color = 0xffFFE189;
						p.color2 = 0xffFFE189;
						...
						p.setRadius(12);
						p.ring = true;
						fireAchievementEvent(AchievementType.CREATE_SATURN);
					}
	...
	private void fireAchievementEvent(AchievementType type) {
